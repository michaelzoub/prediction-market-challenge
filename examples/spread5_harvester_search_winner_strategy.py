...
class Strategy(BaseStrategy):
    """Search winner around the spread>=5 harvester family.

    This is intentionally simple: the search found that modest size, strict
    tox gating, and strong directional one-sidedness outperformed the more
    pathological large-size variants.
    """
    ...
    def on_step(self, state: StepState):
        ...
        if spread < 5 or self.streak < 2:
            return actions
        if self.tox > 0.65 or self.abs_move > 0.52:
            return actions
        ...
        signal = 0.75 * move + 0.65 * self.flow
        quote_buy = inv < 220
        quote_sell = inv > -220
        if signal > 1.2:
            quote_sell = False
