...
class Strategy(BaseStrategy):
    """Reversed variant of paired_momentum_revert strategy.

    Reversal changes:
    - flip trend skew sign (contrarian to the original directional lean),
    - flip paired-fill sizing response.
    """
    ...
    def on_step(self, state: StepState) -> Sequence[Action]:
        ...
        # Reverse the original trend lean.
        trend_skew = 0
        if trend > 0.22:
            trend_skew = -1
        elif trend < -0.22:
        ...
        # Reverse paired-fill response vs the original strategy.
        buy_size = base_size
        sell_size = base_size
        if self.last_fill_imb > 1.0:
            buy_size = min(6.0, base_size + 1.5)
        elif self.last_fill_imb < -1.0:
            sell_size = min(6.0, base_size + 1.5)
