...
class Strategy(BaseStrategy):
    """Spread>=5 harvester v6 — v1 hybrid with VPIN gating, adaptive offset, flow-lean."""
    ...
    def on_step(self, state):
        ...
        self.vpin_buy = 0.93 * self.vpin_buy + 0.07 * state.buy_filled_quantity
        self.vpin_sell = 0.93 * self.vpin_sell + 0.07 * state.sell_filled_quantity
        vpin_total = self.vpin_buy + self.vpin_sell
        self.vpin_imb = abs(self.vpin_buy - self.vpin_sell) / max(0.01, vpin_total)
        ...
        if self.vpin_imb > 0.85 and vpin_total > 0.5:
            self.cool = 2
            return actions
        ...
        offset = 2
        if self.tox > 0.30 or self.abs_move > 0.35:
            offset = 3

        center = int(round(mid - 0.20 * move - 0.35 * self.flow - 0.03 * inv))
        buy_tick = _clip(center - offset)
        sell_tick = _clip(center + offset)
        ...
        flow_signal = 0.5 * move + 0.5 * self.flow
        if abs(flow_signal) > 0.4 and self.tox <= 0.32:
            if flow_signal > 0:
                size_buy = size * 1.3
                size_sell = size * 0.7
            else:
        ...
        if quote_buy and buy_px < ask:
            bq = self._buy_qty(buy_px, size_buy, budget)
            if bq >= 0.01:
        ...
        if quote_sell and sell_px > bid:
            sq = self._sell_qty(sell_px, size_sell, budget, state.yes_inventory)
            if sq >= 0.01:
